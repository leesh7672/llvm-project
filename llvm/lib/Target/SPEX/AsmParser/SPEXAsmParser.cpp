//===- SPEXAsmParser.cpp - Parse SPEX assembly ------------------------===//
//
...
class SPEXOperand : public MCParsedAsmOperand {
...
public:
  SPEXOperand(KindTy Kind, SMLoc StartLoc, SMLoc EndLoc)
  ...
  static std::unique_ptr<SPEXOperand> createToken(StringRef Tok, SMLoc Loc) {
    auto Op = std::make_unique<SPEXOperand>(k_Token, Loc, Loc);
    Op->Tok = Tok;
    return Op;
  }
...
class SPEXAsmParser : public MCTargetAsmParser {
  MCAsmParser &Parser;

#define GET_ASSEMBLER_HEADER
#include "SPEXGenAsmMatcher.inc"
  ...
  bool parseMemOperand(OperandVector &Operands) {
    SMLoc StartLoc = getLexer().getLoc();
    getLexer().Lex(); // '['
    Operands.push_back(SPEXOperand::createToken("[", StartLoc));
    ...
    if (Reg == SPEX::NoRegister)
      return Error(StartLoc, "invalid register in memory operand");
    getLexer().Lex(); // eat rx
    Operands.push_back(
        SPEXOperand::createReg(Reg, RegTok.getLoc(), RegTok.getEndLoc()));

    if (getLexer().is(AsmToken::RBrac)) {
      SMLoc EndLoc = getLexer().getLoc();
      getLexer().Lex();
      Operands.push_back(SPEXOperand::createToken("]", EndLoc));
      return false; // no offset operand
    }

    bool IsMinus = false;
    if (getLexer().is(AsmToken::Plus)) {
      Operands.push_back(SPEXOperand::createToken("+", getLexer().getLoc()));
      getLexer().Lex();
    ...
    Operands.push_back(SPEXOperand::createImm(Expr, StartLoc, ImmEndLoc));
  ...
  bool parseOperand(OperandVector &Operands) {
    ...
    if (Res.isSuccess()) {
      Operands.push_back(SPEXOperand::createReg(RegNo, StartLoc, EndLoc));
      return false;
    }
  ...
  bool parseInstruction(ParseInstructionInfo &Info, StringRef Name,
    ...
    Operands.push_back(SPEXOperand::createToken(Name, NameLoc));
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeSPEXAsmParser() {
  RegisterMCAsmParser<SPEXAsmParser> X(getTheSPEXTarget());
}
